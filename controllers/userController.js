...
let userGetCheckout = async function (req, res, next) {
  try {
    ...
    const userCheck = await user_details.findOne({
      username: req.session.user,
    });
...
let userGetCart = async function (req, res, next) {
  try {
    ...
    res.render("user-cart", {
      ...
      appliedCoupon,
      couponErrorMsg,
      discountAmount: req.session.deducted,
      subT,
      dbWallet: dbWallet.wallet,
    });
    subT = null;
    appliedCoupon = null;
    userdone = null;
  ...
  try {
    ...
    if (req.session.orders.paymentType == "Pay using razorpay") {
      ...
      if (req.session.orders.billAmount < walletCheck.wallet) {
        const hijack = await user_details.findOne({
          username: req.session.user,
        });
        const uss = hijack._id.toString();
        for (var i = 0; i < req.session.userdone.length; i++) {
          req.session.orders.products[i].paymentId = uuidv4();
        ...
        await user_details.updateOne(
          { username: req.session.user },
          { $inc: { wallet: "-" + req.session.orders.billAmount } }
        );
...
let userGetDeleteOne = async function (req, res, next) {
  try {
    ...
    if (parm.status == "Cancelled") {
      ...
      if (
        parm.paymentType == "wallet" ||
        parm.paymentType == "Pay using razorpay"
      ) {
        ...
        const amountAdd =
          walletInc[0].products.price * walletInc[0].products.quantity;
        console.log(amountAdd);
        await user_details.updateOne(
          { username: req.session.user },
          { $inc: { wallet: amountAdd } }
        );
      }
    }
...
let userPostChangeQuantity = async function (req, res, next) {
  try {
    ...
    for (var i = 0; i < price.length; i++) {
      subT =
        subT + parseInt(price[i].quantity) * parseInt(price[i].product.price);
    ...
    if (req.session.appliedCoupon) {
      ...
      if (repeatCheck == null) {
        if (req.session.appliedCoupon.discountType == "percentage") {
          if (subT < parseInt(req.session.appliedCoupon.minPurchase)) {
            console.log(
              `sorry you need to purchase for minimum ${req.session.appliedCoupon.minPurchase} for this product`
            );
            minpur = true;
            var minpursup = req.session.appliedCoupon.minPurchase;
            couponErrorMsg = `sorry you need to purchase for minimum ${req.session.appliedCoupon.minPurchase} for this product`;
            req.session.appliedCoupon.validity = false;
          } else {
            req.session.deducted =
            ...
            if (
              ...
              var maxpursup = req.session.appliedCoupon.maxDiscountAmount;
              couponErrorMsg = `sorry amount greater than ${req.session.appliedCoupon.maxDiscountAmount}`;
              maxpur = true;
              req.session.appliedCoupon.validity = false;
            } else {
    ...
    if (req.session.deducted && minpur == false && maxpur == false) {
      var discount = parseInt(req.session.deducted);
